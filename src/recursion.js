...
var arraySum = function(array) {
  ...
  var currentArrayIndex = array[array.length-1];

  if (currentArrayIndex.length !== undefined) {
   return (arraySum(currentArrayIndex) + arraySum(array.slice(0, array.length-1)));
  }
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
  for (var value in obj) {
    if (typeof obj[value] === 'object') {
      replaceKeysInObj(obj[value], oldKey, newKey);
    }
    if (value === oldKey) {
...
var fibonacci = function(n) {
  ...
  while (count <= n) {
    if (count === 0){
      ...
      var value = array[count-1] + array[count-2];
      array.push(value);
    }
    ...
    if (count === 1 && count === n){
      ...
      var value2 = array[count-1] + array[count-2];
      newArray.push(value2);
      return ((fibonacci(n-1)).concat(newArray));
    }
    count++;
  }
};
